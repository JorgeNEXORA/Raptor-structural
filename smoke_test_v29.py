...
adv = ProjectAdvisor()
adv.project_score(p)
adv.generate_advice(p)
before = store_snapshot(p, "antes_otimizacao")
...
adv.project_score(p)
adv.generate_advice(p)
after = store_snapshot(p, "depois_otimizacao")

lines = build_comparison_text(before, after)
print("changes", changes[:10])
print("comparison_lines", len(lines))
print("\n".join(lines[:15]))
